...
contract ConveyorImp4 {
  ...
  function getAssetFromID(uint assetID) external pure returns (string memory) {
    if (assetID == 12018) {
      ...
      return string.concat(ConveyorAdditional5.getPart(), ConveyorAdditional6.getPart());
    } else if (assetID == 12024) {
      return CB_U_SHAD_P4;
    } else if (assetID == 12025) {
      return string.concat(ConveyorAdditional7.getPart(), ConveyorAdditional8.getPart());
    } else {
      return "";
    }
  }
