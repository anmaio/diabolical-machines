...
library Environment {
  ...
  function getColourIndex(uint baseValue) internal pure returns(uint) {
    // uint baseValue = GridHelper.constrainToHex(noiseArray[GridHelper.getRandByte(rand, 3)] + baseline);
    // uint topEyesDigits = GridHelper.constrainToHex(_noise.getNoiseArrayOne()[GridHelper.getRandByte(rand, 3)] + baseline);
    ...
    // uint[] memory percentages = GridHelper.setUintArrayFromString(DEGRADED_COLOUR_PERCENTAGES, 8, 3);
    // if (state == 1) {
    //   percentages = GridHelper.setUintArrayFromString(BASIC_COLOUR_PERCENTAGES, 8, 3);
    // } else {
    //   percentages = GridHelper.setUintArrayFromString(EMBELLISHED_COLOUR_PERCENTAGES, 8, 3);
    // }
    // uint total = 0;
