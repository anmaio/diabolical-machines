...
library Nose {
  ...
  string internal constant MOVING_PANEL = "";
  ...
  function getHoles(uint[6] memory holeDistribution) internal pure returns (string memory) {
    ...
    for (uint i = 0; i < holeDistribution.length; i++) {
      ...
      if (holeDistribution[i] == 0) {
        output = string.concat(output, getArrangement(xOffset, yOffset, HOLES_5));
      // 4 holes
      } else if (holeDistribution[i] == 1) {
        output = string.concat(output, getArrangement(xOffset, yOffset, HOLES_4));
      }
    }
    return output;
  }
