...
library Nose {
  ...
  function getMachine(uint[6] memory holeDistribution, bytes memory digits) internal pure returns (string memory) {
    ...
    string memory output = string.concat("<g id='m_a_limit33-u-a_to' style='animation-play-state: running;'>", MOVING_PANEL, "</g>");
    return string.concat(CommonSVG.groupTransform("-212", "-240", getHoles(holeDistribution, digits)), output);
  }
}