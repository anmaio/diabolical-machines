...
contract Conveyorbelt {
  ...
  function getMachine() external pure returns (string memory) {
    return CommonSVG.groupTransform("-468", "-630", string.concat(GridHelper.repeatGivenObject(conveyorbeltAdditional1.getPipes(), 9, PIPE_POSITIONS), conveyorbeltAdditional1.getHole(), conveyorbeltAdditional1.getBeltOpen(), GridHelper.repeatGivenObject(conveyorbeltAdditional1.getBeltDisc(), 10, DISC_POSITIONS), conveyorbeltAdditional1.getBeltClose(), conveyorbeltAdditional2.getFloob(), conveyorbeltAdditional2.getSlicer(), conveyorbeltAdditional2.getLever()));
  }
}